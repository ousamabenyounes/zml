...
pub fn Pool(comptime Job: type) type {
    return struct {
                ...
                while (true) {
                    ...
                    switch (status) {
                        ...
                        .retry => {
                            ...
                            pool.sleepBeforeRetry(io, attempt) catch {
                                job.batch.state.fail(error.RetriesExhausted);
                                break;
                            };
        ...
        fn sleepBeforeRetry(pool: *Self, io: std.Io, attempt: usize) std.Io.Cancelable!void {
            // FullJitter algorithm
            const max_delay = std.Io.Duration.fromNanoseconds(@min(
            ...
            const delay_ns = pool.prng.random().intRangeAtMost(i96, 0, max_delay.toNanoseconds());
