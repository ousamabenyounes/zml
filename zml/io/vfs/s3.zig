...
pub const S3 = struct {
    const ParallelRead = struct {
        ...
        fn readFailureStatus(head: std.http.Client.Response.Head) ?parallel_read.Status {
            return switch (head.status) {
                .request_timeout, .too_many_requests => .retry,
                else => if (head.status.class() == .server_error) .retry else null,
            };
        }
        ...
        const Job = struct {
            ...
            pub fn perform(job: Job, client: *std.http.Client) anyerror!parallel_read.Status {
                ...
                req.sendBodiless() catch |err| switch (err) {
                    error.WriteFailed => {
                        log.err("Failed to send headers: {}", .{err});
                        return .retry;
                    },
                };
                ...
                if (res.head.status != .partial_content and res.head.status != .ok) {
                    ...
                    return switch (res.head.status) {
                        .request_timeout, .too_many_requests => .retry,
                        else => if (res.head.status.class() == .server_error) .retry else error.RequestFailed,
                    };
                }
